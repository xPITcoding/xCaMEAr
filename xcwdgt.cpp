    ...
    , ui(new Ui::xCWdgt)
{
    ...
    pInfoPanelAnimation->setStartValue(QRect(575, 5, 0, 336));
    pInfoPanelAnimation->setEndValue(QRect(313, 5, 260, 336));
    pSettingsPanelAnimation = new QPropertyAnimation(ui->pSettingsPanel, "geometry");
    pSettingsPanelAnimation->setDuration(500);
    pSettingsPanelAnimation->setStartValue(QRect(575, 340, 0, 161));
    pSettingsPanelAnimation->setEndValue(QRect(0, 340, 573, 161));
    ui->pStatusPanel->setGeometry(QRect(575, 5, 0, 336));
    ui->pSettingsPanel->setGeometry(QRect(575, 340, 0, 161));
...
void xCWdgt::openSettings(bool b)
{
    ...
    {
        pSettingsPanelAnimation->setEndValue(QRect(575, 340, 0, 161));
        pSettingsPanelAnimation->setStartValue(QRect(0, 340, 573, 161));
        pSettingsPanelAnimation->start();
        pSlideEffect->play();
    }
}
...
void xCWdgt::openInfo(bool b)
{
    ...
    {
        pInfoPanelAnimation->setEndValue(QRect(575, 5, 0, 336));
        pInfoPanelAnimation->setStartValue(QRect(313, 5, 260, 336));
        pInfoPanelAnimation->start();
        pSlideEffect->play();
    }
}
...
void xCWdgt::dispImage(QImage* img)
{
   ...
   pCamThread->isStreaming() ?
               pain.drawText(QPointF(20,20),QString("streaming %1 fps").arg(1.0f/((float)_lastTime.msecsTo(QTime::currentTime()))*1000.f,0,'f',0)) :
               pain.drawText(QPointF(20,20),QString("%1 fps").arg(1.0f/((float)_lastTime.msecsTo(QTime::currentTime()))*1000.f,0,'f',0));
   pain.end();
...
void xCWdgt::cameraParameterChanged()
{
    ...
    if (pCamera && pCamConfig)
    {
        try
        {
            ...
            if (CAMERA_IMAGE_FORMAT==QImage::Format_Grayscale8)
                CEnumerationPtr(control.GetNode("PixelFormat"))->FromString("Mono8");
            else
                CEnumerationPtr(control.GetNode("PixelFormat"))->FromString("Mono12");
        }
        catch (...)
        {
        ...
        try
        {
            ...
            if (pCamera->IsPylonDeviceAttached())
            {
                ...
                if (CAMERA_IMAGE_FORMAT==QImage::Format_Grayscale8)
                    ...
                    pCamera->PixelFormat.SetValue(PixelFormat_Mono12);
